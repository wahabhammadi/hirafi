        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-6">
                <div class="p-4">
                    ...
                    <form action="{{ route('admin.offres.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                        ...
                        <div>
                            <x-input-label for="craftsman" :value="__('اسم الحرفي')" />
                            <x-text-input id="craftsman" type="text" name="craftsman" class="mt-1 block w-full" value="{{ request('craftsman') }}" />
                        </div>
                        <div>
                        ...
                        <div class="md:col-span-4 flex justify-end">
                            ...
                            @if(request()->anyFilled(['status', 'commande_id', 'craftsman', 'delivery_date']))
                                <a href="{{ route('admin.offres.index') }}" class="mr-2 inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                    {{ __('إعادة ضبط') }}
                                </a>
                            @endif
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                <div class="p-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">{{ __('قائمة العروض') }}</h3>
                            ...
                            <tbody class="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $offre->user->email }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.offres.show', $offre) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 ml-3">
                                            {{ __('عرض') }}
                                        </a>
                                        
                                        @if($offre->status == 'pending')
                                        <form action="{{ route('admin.offres.update-status', $offre) }}" method="POST" class="inline-block ml-3">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="status" value="accepted">
                                            <button type="submit" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300" onclick="return confirm('{{ __('هل أنت متأكد من قبول هذا العرض؟') }}')">
                                                {{ __('قبول') }}
                                            </button>
                                        </form>
                                        ...
                                        <form action="{{ route('admin.offres.update-status', $offre) }}" method="POST" class="inline-block ml-3">
                                            ...
                                            <button type="submit" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300" onclick="return confirm('{{ __('هل أنت متأكد من رفض هذا العرض؟') }}')">
                                                {{ __('رفض') }}
                                            </button>
                                        </form>
                                        @endif
                                        ...
                                        <form action="{{ route('admin.offres.update-status', $offre) }}" method="POST" class="inline-block ml-3">
                                            ...
                                            <button type="submit" class="text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300" onclick="return confirm('{{ __('هل أنت متأكد من إعادة هذا العرض إلى حالة الانتظار؟') }}')">
                                                {{ __('إعادة للانتظار') }}
                                            </button>
                                        </form>
                                        @endif
